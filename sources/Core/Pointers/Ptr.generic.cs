...
public unsafe struct Ptr<T, BaseType>(T* ptr) : IPointerToMutable
    where T : unmanaged
    where BaseType : unmanaged
{
    ...
    /// </summary>
    /// <param name="ptr"></param>
    public static implicit operator PtrToConst<T, BaseType>(Ptr<T, BaseType> ptr) => new(ptr.Pointer);

    /// <summary>
    /// Creates a <see cref="Ptr"/> from the pointer
    /// </summary>
    /// <param name="ptr"></param>
    public static implicit operator Ptr(Ptr<T, BaseType> ptr) => new(ptr.Pointer, Depth);

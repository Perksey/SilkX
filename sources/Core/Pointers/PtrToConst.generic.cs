...
public unsafe struct PtrToConst<T, BaseType>(T* ptr, GCHandle? handle = null) : IPointerToMutable, IDisposable
    where T : unmanaged
    where BaseType : unmanaged
{
    ...
    /// </summary>
    /// <param name="handle"></param>
    public PtrToConst(GCHandle handle)
        : this(null)
    {
        Handle = handle;
    ...
    /// </summary>
    /// <param name="ptr"></param>
    public static explicit operator Ptr<T, BaseType>(PtrToConst<T, BaseType> ptr) => new(ptr.Pointer, ptr.Handle);

    /// <summary>
    /// Creates a <see cref="PtrToConst{T, BaseType}"/> from the pointer
    /// </summary>
    /// <param name="ptr"></param>
    public static implicit operator PtrToConst(PtrToConst<T, BaseType> ptr) => new(ptr.Pointer, ptr.Handle, Depth);
    ...
    /// <summary>
    /// Creates a <see cref="PtrToConst{T, BaseType}"/> from a BaseType array
    /// Only valid for depth 1 pointers 
    /// </summary>
    /// <param name="array"></param>
    public static implicit operator PtrToConst<T, BaseType>(BaseType[] array) => Depth != 1 ? throw new InvalidCastException() : new(GCHandle.Alloc(array, GCHandleType.Pinned));

    /// <summary>
    /// Creates a <see cref="PtrToConst{T, BaseType}"/> from a jagged 2D BaseType array
    /// Only valid for depth 2 pointers 
    /// </summary>
    /// <param name="array"></param>
    public static implicit operator PtrToConst<T, BaseType>(BaseType[][] array) => Depth != 2 ? throw new InvalidCastException() : SilkMarshal.JaggedArrayToPointerArray<BaseType>(array);
    ...
    /// </summary>
    /// <param name="array"></param>
    public static implicit operator PtrToConst<T, BaseType>(BaseType[][][] array) => Depth != 3 ? throw new InvalidCastException() : SilkMarshal.JaggedArrayToPointerArray<BaseType>(array);

    /// <summary>
    /// Creates a <see cref="PtrToConst{T, BaseType}"/> from a BaseType span
    /// Only valid for depth 1 pointers 
    /// </summary>
    /// <param name="span"></param>
    public static implicit operator PtrToConst<T, BaseType>(Span<BaseType> span)
    {
        if (Depth != 1)
            throw new InvalidCastException();
    ...
    /// <summary>
    /// Creates a <see cref="PtrToConst{T, BaseType}"/> from a jagged 2D BaseType span
    /// Only valid for depth 2 pointers 
    /// </summary>
    /// <param name="span"></param>
    public static implicit operator PtrToConst<T, BaseType>(Span<BaseType[]> span) => Depth != 2 ? throw new InvalidCastException() : new(GCHandle.Alloc(span.GetPinnableReference(), GCHandleType.Pinned));
    ...
    /// </summary>
    /// <param name="span"></param>
    public static implicit operator PtrToConst<T, BaseType>(Span<BaseType[][]> span) => Depth != 3 ? throw new InvalidCastException() : new(GCHandle.Alloc(span.GetPinnableReference(), GCHandleType.Pinned));
    ...
    /// </summary>
    /// <param name="span"></param>
    public static implicit operator PtrToConst<T, BaseType>(ReadOnlySpan<BaseType> span)
    {
        if (Depth != 1)
            throw new InvalidCastException();
        return new(*(T**)SilkMarshal.SpanRefToPtr<BaseType>(ref span), GCHandle.Alloc(span.GetPinnableReference(), GCHandleType.Pinned));
    ...
    /// </summary>
    /// <param name="span"></param>
    public static implicit operator PtrToConst<T, BaseType>(ReadOnlySpan<BaseType[]> span) => Depth != 2 ? throw new InvalidCastException() : new(GCHandle.Alloc(span.GetPinnableReference(), GCHandleType.Pinned));

    /// <summary>
    /// Creates a <see cref="PtrToConst{T, BaseType}"/> from a jagged 3D BaseType span
    /// Only valid for depth 3 pointers 
    /// </summary>
    /// <param name="span"></param>
    public static implicit operator PtrToConst<T, BaseType>(ReadOnlySpan<BaseType[][]> span) => Depth != 3 ? throw new InvalidCastException() : new(GCHandle.Alloc(span.GetPinnableReference(), GCHandleType.Pinned));
    ...
    /// <summary>
    /// Creates a <see cref="PtrToConst{T, BaseType}"/> from a string
    /// Only valid for depth 1 pointers
    /// </summary>
    /// <param name="str"></param>
    public static implicit operator PtrToConst<T, BaseType>(string str)
    {
        if ((
    ...
    /// <summary>
    /// Creates a <see cref="PtrToConst{T, BaseType}"/> from a span of strings
    /// Only valid for depth 2 pointers
    /// </summary>
    /// <param name="span"></param>
    public static implicit operator PtrToConst<T, BaseType>(Span<string> span)
    {
        ...
        return new PtrToConst<T, BaseType>(GCHandle.Alloc(SilkMarshal.StringArrayToArray(span, sizeof(BaseType)), GCHandleType.Pinned));
    }

    /// <summary>
    /// Creates a <see cref="PtrToConst{T, BaseType}"/> from a span of string arrays
    /// Only valid for depth 3 pointers
    /// </summary>
    /// <param name="span"></param>
    public static implicit operator PtrToConst<T, BaseType>(Span<string[]> span)
    {
        if (PtrToConst<T, BaseType>.Depth != 3
            || (typeof(BaseType) != typeof(byte)
            && typeof(BaseType) != typeof(sbyte)
            && typeof(BaseType) != typeof(char)
            && typeof(BaseType) != typeof(short)
    ...
    /// </summary>
    /// <param name="array"></param>
    public static implicit operator PtrToConst<T, BaseType>(BaseType*[] array) => PtrToConst<T, BaseType>.Depth != 2
            ? throw new InvalidCastException()
            : new PtrToConst<T, BaseType>(GCHandle.Alloc(array, GCHandleType.Pinned));
    ...
    /// </summary>
    /// <param name="array"></param>
    public static implicit operator PtrToConst<T, BaseType>(BaseType*[][] array) => PtrToConst<T, BaseType>.Depth != 3
            ? throw new InvalidCastException()
            : new PtrToConst<T, BaseType>(GCHandle.Alloc(array, GCHandleType.Pinned));
    ...
    public static implicit operator PtrToConst<T, BaseType>(string[] strings)
    {
        if (PtrToConst<T, BaseType>.Depth != 2
            && typeof(BaseType) != typeof(byte)
            && typeof(BaseType) != typeof(sbyte)
        ...
        return new PtrToConst<T, BaseType>(GCHandle.Alloc(SilkMarshal.StringArrayToArray(strings, sizeof(BaseType)), GCHandleType.Pinned));
    }
    ...
    /// </summary>
    /// <param name="span"></param>
    public static implicit operator PtrToConst<T, BaseType>(ReadOnlySpan<string> span)
    {
        if (PtrToConst<T, BaseType>.Depth != 2
            && typeof(BaseType) != typeof(byte)
            && typeof(BaseType) != typeof(sbyte)

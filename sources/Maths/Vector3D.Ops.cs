...
namespace Silk.NET.Maths
{
    ...
    public static class Vector3D
    {
        ...
        public static Vector3D<T> Abs<T>(Vector3D<T> value)
            where T : unmanaged, INumber<T> => new(Scalar.Abs(value.X), Scalar.Abs(value.Y), Scalar.Abs(value.Z));
        ...
        public static Vector3D<T> Divide<T>(Vector3D<T> left, T divisor)
            where T : unmanaged, INumber<T> => left / divisor;
        ...
        public static T Dot<T>(Vector3D<T> vector1, Vector3D<T> vector2)
            where T : unmanaged, INumber<T> => Scalar.Add(
        ...
        public static Vector3D<T> SquareRoot<T>(Vector3D<T> value)
            where T : unmanaged, INumber<T> => new(Scalar.Sqrt(value.X), Scalar.Sqrt(value.Y), Scalar.Sqrt(value.Z));
